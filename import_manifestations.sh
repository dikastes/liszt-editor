...
uv run manage.py bib_import
echo importing 1d autographs
uv run manage.py import_manifestations_from_csv "${CSV_PATH_PREFIX}liszt_1d_long.csv" -st autograph
echo importing 2 copies
uv run manage.py import_manifestations_from_csv "${CSV_PATH_PREFIX}liszt_2_long.csv" -st copy
...
uv run manage.py import_gnd_ids "${CSV_PATH_PREFIX}gnd_ids.csv"
echo importing additional gnd ids
uv run manage.py import_gnd_ids "${CSV_PATH_PREFIX}additional_gnd_ids.csv"
...
uv run manage.py import_empty_libraries
