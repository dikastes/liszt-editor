...
def render_editor_mode_warning(context):
    ...
    if settings.EDITOR_MODE.lower() == 'production':
        return ''
    return mark_safe(f'<div class="bg-primary text-sm xl:text-base text-primary-content px-5 flex"><div class="flex-1">{prefix}{mode}</div><div class="flex-0">{message}</div></div>')
